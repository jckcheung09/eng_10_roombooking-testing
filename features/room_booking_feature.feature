Feature: Logging in as user and admin
  ...
  @incorrect_login
  Scenario Outline: As I enter incorrect login details I should be given an error
    Given I am on the login page
    When I input incorrect user login username <username>
    And I input incorrect user login password <password>
    And I click login
    Then I should be given an error message <error>

  Example:
  | username | password | error |
  | test | test | test |

Feature: User Room Booking
  As a user
  I want to be able to log in
  ...
  Scenario Outline: As a user I book a room with incorrect requirements
    Given I am logged in as a user
    When I click to book a room
...
Feature: Admin Room Management
  As an admin
  I want to be able to manage the rooms available for bookings
  So that I can CRUD rooms based on real life resources
  ...
  Scenario Outline: Creation of a new room should not be allowed with incorrect details
    Given I am logged in as an admin
    When I access the rooms resource
    And I click create a new room
    And I enter invalid details <details>
    And I click save
    Then A new room creation error message should be given <error>
...
Feature: Admin User Management
  As an admin
  I want to be able to manage current users
  So that I can add and remove admin status for different users
...
Feature: Admin Booking Management
  As an admin
  I want to be able to manage bookings
  So that I can alter and view bookings as required
